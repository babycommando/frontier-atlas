...
void main() {
  ...
  vec2 radial_uv = uv;
  radial_uv.x *= u_resolution.x / u_resolution.y;
  ...
  if (u_shape < 1.5) {
    ...
    vec2 ripple_uv = radial_uv;
    float dist = length(ripple_uv);
    float waves = sin(pow(dist, 1.7) * 7. - 3. * t) * .5 + .5;
    shape = waves;

  } else if (u_shape < 6.5) {
    vec2 swirl_uv = radial_uv;
    float l = length(swirl_uv);
    float angle = 6. * atan(swirl_uv.y, swirl_uv.x) + 4. * t;
    float twist = 1.2;
    float offset = pow(max(l, 0.0001), -twist) + angle / TWO_PI;
    float mid = smoothstep(0., 1., pow(l, twist));
    shape = mix(0., fract(offset), mid);

  } else {
    vec2 sphere_uv = radial_uv * 2.;
    float d = 1. - pow(length(sphere_uv), 2.);
    vec3 pos = vec3(sphere_uv, sqrt(max(d, 0.0)));
...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      const displayWidth =
        canvas.clientWidth || (typeof width === "number" ? width : 1);
      const displayHeight =
        canvas.clientHeight || (typeof height === "number" ? height : 1);
